...
pub unsafe fn to_value(ptr: *mut unqlite_value) -> Option<Value> {
    ...
    if ptr.is_null() { 
        ...
        if size < 0 { None }
            ...
            if walk_result != UNQLITE_OK { None }
            else {
        ...
        if size < 0 { None }
            ...
            let walk_result: c_int = unqlite_array_walk(ptr, Some(array_walk), values as *mut c_void);
            debug_assert_eq!(UNQLITE_OK, walk_result);
            if walk_result != UNQLITE_OK { None }
            else {
...
unsafe fn value_to_string(ptr: *mut unqlite_value) -> Option<String> {
    ...
    if cstr.is_null() { None }
        ...
        let string = String::from_utf8_lossy(slice)
                            .to_string(); // clone bytes
        Some( string )
    }
}

unsafe extern "C" fn array_walk(_: *mut unqlite_value, 
                                value: *mut unqlite_value, 
                                collection: *mut c_void) -> i32 
{
...
unsafe extern "C" fn map_walk(key: *mut unqlite_value, 
                              value: *mut unqlite_value, 
                              collection: *mut c_void) -> i32 
{
    let collection = collection as *mut Map;
    let both = value_to_string(key).and_then(|k| 
               to_value(value).map(|v| (k, v)));
    match both {
        Some((k, v)) => {
