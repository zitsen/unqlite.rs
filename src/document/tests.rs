...
fn output_std() {
    ...
    db.compile(prog).and_then(|mut vm| {
        vm.output_to_stdout()?;
        vm.report_errors_to_output()?;
        vm.exec_void()?;
        vm.output_length().map(
            |len| assert_eq!(18, len))
    }).unwrap()
}
...
fn dump_vm_instructions() {
    let db = UnQLite::create_in_memory();
    let prog = "print 'hello '; print 'world'..'!';";
    db.compile(prog).and_then(|vm| vm.dump())
      .unwrap();
}
...
fn output_extract() {
    ...
    db.compile(prog).and_then(|mut vm| {
        vm.exec().and_then(|_|
            vm.extract_output().map(
                |msg| assert_eq!("hello world", String::from_utf8_lossy(msg)) 
            )
        )
    }).unwrap();
}
...
fn exec_result() {
    ...
    db.compile("print 'will return null';")
      .map(|mut vm| { assert_eq!(Some(Value::Null), vm.exec().unwrap()); })
      .unwrap();

    db.compile("return -42;")
      .map(|mut vm| { assert_eq!(Some(Value::Int(-42)), vm.exec().unwrap()); })
      .unwrap();

    db.compile("return 12.345;")
      .map(|mut vm| { assert_eq!(Some(Value::Real(12.345)), vm.exec().unwrap()); })
      .unwrap();
    
    db.compile("return 'hello';")
      .map(|mut vm| { assert_eq!(Some(Value::String("hello".to_string())), vm.exec().unwrap()); })
      .unwrap();

    db.compile("return [1,2,'test',[3,4.5,null]];")
      .map(|mut vm| { 
          let test = Value::Array(vec![ Value::Int(1), Value::Int(2), Value::string("test"), Value::Array(vec![Value::Int(3), Value::Real(4.5), Value::Null]) ]);
          assert_eq!(Some(test), vm.exec().unwrap()); })
      .unwrap();

    db.compile("return {'key_one': 123, 'key_2': 'string_value', 'key_3': [1,2,3], 'key_4': {'sub_key': 'sub_value', 'sub_key2': 42}, 'key_5': 1.2};")
      .map(|mut vm| { 
          vm.exec()
            .map(|opt| opt.unwrap())
            .map(|result| {
               ...
               if let Some(map) = map {
                   ...
                   assert_eq!(Value::Array(vec![Value::Int(1), Value::Int(2), Value::Int(3)]), map["key_3"]);
