...
use ffi::{ unqlite_vm, unqlite_value, 
           unqlite_compile,
...
use UnQLite;
use super::vm_value::{Value, to_value};
use error::{Result, Wrap};

/// Jx9 script compiler Interface.
///
/// The Document store interface to UnQLite which is used to store JSON docs (i.e. Objects, Arrays, Strings, etc.) in the database is powered by the 
...
impl UnQLiteVm {
    ...
    pub fn dump(&self) -> Result<()> {
        wrap_raw!(self, vm_dump, Some(callback_to_stdout), null_mut::<c_void>())
    }
    ...
    pub fn output_to_channel(&mut self) -> Result<mpsc::Receiver<Vec<u8>>> {
        ...
        wrap_raw!(self, vm_config, UNQLITE_VM_CONFIG_OUTPUT, 
    ...
    pub fn extract_output(&self) -> Result<&[u8]> {
        ...
        wrap_raw!(self, vm_config, UNQLITE_VM_CONFIG_EXTRACT_OUTPUT, &ptr, &mut len)
            .map(|_| unsafe { 
    ...
    pub fn import_path<T: AsRef<str>>(&self, path: T) -> Result<()> {
        ...
        wrap_raw!(self, vm_config, UNQLITE_VM_CONFIG_IMPORT_PATH, path.as_ptr())
    }
    ...
    /// Set a recursion limit to the running script. That is, a function may not call itself (recurse) more than this limit. 
    /// If this limit is reached then the virtual machine abort the call and null is returned to the caller instead of the 
    /// function return value.
    /// Should be called before `exec()` method.
    pub fn recursion_depth(&self, max_depth: i32) -> Result<()> {
        wrap_raw!(self, vm_config, UNQLITE_VM_CONFIG_RECURSION_DEPTH, max_depth)
    }
        ...
        where K: AsRef<[u8]>, V: AsRef<[u8]>
    {
        ...
        wrap_raw!(self, vm_config, UNQLITE_VM_CONFIG_ENV_ATTR, 
                                   key.as_ptr(), 
                                   value.as_ptr(), 
                                   value.len() as i32)
    }
    ...
    fn new_variable(&self, value: Value) -> Result<RawValue> {
        let raw: *mut unqlite_value = 
            unsafe {
                if value.is_scalar() { unqlite_vm_new_scalar(self.as_raw_mut_ptr()) }
                else { unqlite_vm_new_array(self.as_raw_mut_ptr()) }
            };
        ...
        match value {
            ...
            Value::String(x) => { wrap!(value_string, raw, CString::new(x.as_bytes()).unwrap().as_ptr(), x.len() as i32)? }
            Value::Array(array) => {
                for x in array {
            ...
            Value::Object(map) => {
                for (key, value) in map {
                    ...
                    wrap!(array_add_strkey_elem, raw, raw_key.as_ptr(), raw_value.as_ptr())?;
                }
            }
        }
    ...
    // This option is useful if you want to pass information from the outside environment to the target Jx9 script.
    pub fn add_variable<T: Into<Vec<u8>>>(&mut self, name: T, value: Value) -> Result<()> {
        ...
        wrap_raw!(self, vm_config, UNQLITE_VM_CONFIG_CREATE_VAR, name.as_ptr(), value.as_ptr())
    }
    ...
    pub fn extract_variable<T: AsRef<[u8]>>(&self, name: T) -> Option<Value> {
        ...
        unsafe {
            to_value( unqlite_vm_extract_variable(self.as_raw_mut_ptr(), name.as_ptr()) )
        }
    }
...
impl Drop for UnQLiteVm {
    ...
    fn drop(&mut self) {
        unsafe { unqlite_vm_release(self.as_raw_mut_ptr()); }
    }
}
...
extern "C" fn callback_to_channel(data: *const c_void, len: u32, sender: *mut c_void) -> i32 {
    ...
    match sender.send(msg) {
        ...
        // This error means that the channel receiver is disconnected and data will never be received.
...
impl Drop for RawValue {
    fn drop(&mut self) {
        unsafe { unqlite_vm_release_value(self.host, self.raw); }
    }
}
